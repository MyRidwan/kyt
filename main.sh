...
green='\e[0;32m'
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  ${green}Welcome To RidWan Project Script Installer ${NC}${YELLOW}(${NC} ${red}Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     ${green}This Will Quick Setup VPN Server On Your Server${NC}"
echo -e "         ${green}Auther : RidWan ${NC}${YELLOW}(${NC} ${red}Kyt Project ${NC}${YELLOW})${NC}"
echo -e "      ${green} © Recode By Geo Project & Ridwan${YELLOW}(${NC} ${RED}2023${NC} ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""

# // Checking Os Architecture
if [[ $( uname -m | awk '{print $1}' ) == "x86_64" ]]; then
    echo -e "${OK} ${BLUE}Your Architecture Is Supported${NC} ( ${green}$( uname -m )${NC} )"
else
    echo -e "${EROR} ${BLUE}Your Architecture Is Not Supported${NC} ( ${YELLOW}$( uname -m )${NC} )"
    exit 1
fi

# // Checking System
if [[ $( cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g' ) == "ubuntu" ]]; then
    echo -e "${OK} ${BLUE}Your OS Is Supported${NC} ( ${green}$( cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/PRETTY_NAME//g' )${NC} )"
elif [[ $( cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g' ) == "debian" ]]; then
    echo -e "${OK} ${BLUE}Your OS Is Supported${NC} ( ${green}$( cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/PRETTY_NAME//g' )${NC} )"
else
    echo -e "${EROR} ${BLUE}Your OS Is Not Supported${NC} ( ${YELLOW}$( cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/PRETTY_NAME//g' )${NC} )"
    exit 1
fi

# // IP Address Validating
if [[ $IP == "" ]]; then
    echo -e "${EROR} ${BLUE}IP Address${NC} ( ${YELLOW}Not Detected${NC} )"
else
    echo -e "${OK} ${BLUE}IP Address${NC} ( ${green}$IP${NC} )"
fi
...
VERSIONSC () {
    GEOVPN=V3.0
    IZINVERSION=$(curl https://raw.githubusercontent.com/myridwan/izinvps/ipuk/ip | grep $MYIP | awk '{print $6}')
...
VALIDITY () {
 ...
 VERSIONSC
    ...
    echo -e "\e[31mTolong Renew Script di  @Kytxz\e[0m"
    exit 0
fi
}
...
echo -e "             ${green}Telegram t.me/Kytxz "
echo -e "             WhatsApp wa.me/6281774970898"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
rm -f main.sh
exit 0
